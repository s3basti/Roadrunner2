...
# m1pos_offset = todeg((my_drive.axis1.encoder.pos_estimate)/4.5) + (18 + pitch)
# m1pos_offset = todeg((my_drive.axis1.encoder.pos_estimate)/4.5)
# m1pos_offset = (my_drive.axis1.encoder.pos_estimate)/4.5 + tocounts(pitch)
...
while True:
    try:
        ...
        theta0 = todeg(m0pos - m0pos_offset)
        theta1 = todeg(-m1pos + m1pos_offset) #(wheel angle)
        # theta = todeg(-m1pos) + m1pos_offset

        # Get speed (for data only)
        m0speed = my_drive.axis0.encoder.vel_estimate
        m1speed = my_drive.axis1.encoder.vel_estimate

        # Convert speed to deg/s from counts/s
        speed0 = m0speed/8192*360
        speed1 = m1speed/8192*360 # Deg/s
        ...
        # beta = pitch - theta
        beta0 = bindto180(bindto360(pitch -theta0 - beta0))
        beta1 = bindto180(bindto360(pitch + theta1 + beta0)) ######

        # print('Counts: %i Degrees: %.4f Theta: %.4f Pitch: %.3f Beta: %.4f Speed: %.4f' % (m1pos,m1pos_deg,theta,pitch,beta,m1speed))
        # print('Degrees: %.4f Theta: %.4f Pitch: %.3f BetaDiff: %.4f BetaSum: %.4f' % (m1pos_deg,theta,pitch,beta,beta_sum))
        # print('Theta: %.4f Pitch: %.3f Beta: %.4f Speed: %.4f' % (theta,pitch,beta,speed))
        print('Theta0: %.4f Theta1: %.4f Pitch: %.3f Beta0: %.4f Beta1: %.4f Speed0: %.4f Speed1: %.4f' % (theta0, theta1, pitch, beta0, beta1, speed0, speed1))
